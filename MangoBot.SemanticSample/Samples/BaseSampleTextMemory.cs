...
public class BaseSampleTextMemory
{
    ...
    public static async Task RunAsync(CancellationToken cancellationToken = default)
    {
        ...
        store = await RedisMemoryStoreFactory.CreateSampleRedisMemoryStoreAsync();
    ...
    private static async Task RunWithStoreAsync(IMemoryStore memoryStore, CancellationToken cancellationToken)
    {
        ...
        builder.AddOpenAITextEmbeddingGeneration(Constants.OpenAIEmbeddingModel, Constants.OpenAIToken);
        ...
        var embeddingGenerator = new OpenAITextEmbeddingGenerationService(Constants.OpenAIEmbeddingModel, Constants.OpenAIToken);
        ...
        var memoryPlugin = kernel.CreatePluginFromObject( new TextMemoryPlugin(textMemory));
        ...
        var cl =  textMemory.AsJson();
        Console.WriteLine($"tm: {cl}");
        ...
        // cl = await textMemory.GetCollectionsAsync();
        // Console.WriteLine($"tm: {cl.Count()}");
...
//
//         var aboutMeOracle =
//             kernel.CreateFunctionFromPrompt(RecallFunctionDefinition, new OpenAIPromptExecutionSettings() { MaxTokens = 100 });
//
